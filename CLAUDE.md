...
- **Never run** `darwin-rebuild switch/build`, `nix flake update`, or the `nixswitch` alias automatically. They are slow and may prompt for credentials. After making config changes, ask the user to run `nixswitch` themselves.
- **New files must be `git add`ed before `nixswitch`.** This repo is a flake, and Nix only sees files tracked by git. If a change introduces a new file (e.g. a new dotfile, prompt, cheatsheet, module), offer to run `git add <paths>` for the user *before* prompting them to `nixswitch`. Modified-but-tracked files are fine without staging. When in doubt, run `git status` and stage anything under `??`.
...
- **Personal / per-machine info (PII) belongs in `~/.config/dave_nix/private.nix`, never in the public repo.** This includes the macOS short username, real name, email, and anything else that identifies the user or the machine. The flake imports this file at evaluation time (impure read; `nixswitch` passes `--impure`) and threads the resulting `private` attrset to home-manager via `extraSpecialArgs`. The schema lives in `private.nix.example` at the repo root. When adding a new piece of personal/per-machine config:
  ...
  3. Tell the user to add the corresponding key to their own `~/.config/dave_nix/private.nix` before the next `nixswitch`.
...
- The `nixupdate-wrapped` shell alias launches an agent (Pi) that reads
